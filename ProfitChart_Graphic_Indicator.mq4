...
int OnInit()
{
   ...
   if(!IsDllsAllowed())
   {
      Alert("全履歴を自動的にロードするには、DLLの使用を許可してください。\n\n",
            "許可方法:\n",
   ...
   else
   {
      ...
      Print("全履歴のロードを要求しました。数秒お待ちください...");
      Sleep(2000);  // 履歴のロードを待つ
   }
#endif
...
string GenerateHTMLContent(const TradeStatistics &stats)
{
   ...
   int ti;
   for(ti = 0; ti < trade_count && ti < 1000; ti++)  // Display up to 1000 trades
   {
      string profit_class = (g_trades[ti].profit >= 0) ? "profit" : "loss";
      html += "<tr>\n";
      html += "<td>" + IntegerToString(ti + 1) + "</td>\n";
      html += "<td>" + TimeToString(g_trades[ti].time, time_format) + "</td>\n";
      html += "<td class='" + profit_class + "'>" + FormatNumberWithCommas(g_trades[ti].profit) + "</td>\n";
      html += "<td>" + FormatNumberWithCommas(g_trades[ti].cumulative) + "</td>\n";
      html += "</tr>\n";
   }
   ...
   int xi;
   for(xi = 0; xi < trade_count; xi++)
   {
      if(xi > 0) html += ", ";
      html += "'" + IntegerToString(xi + 1) + "'";
   }
   html += "],\n";
   ...
   int yi;
   for(yi = 0; yi < trade_count; yi++)
   {
      if(yi > 0) html += ", ";
      html += DoubleToString(g_trades[yi].cumulative, 2);
   }
   html += "],\n";
   ...
   {
      if(di > 0) html += ", ";
      html += "'" + TimeToString(g_trades[di].time, TIME_DATE|TIME_SECONDS) + "'";
   }
